...
int main ( int argc , char ** argv ) {
  ...
  if ( argc >= 2 ) {
    ...
    if (parasfile.is_open()){
      ...
      // mean, std, min, max
      parasfile << "car_maxAccel," << (x[0]/100) << ",0.2,2.6,3.4\n" ;
      parasfile << "car_reactionTime," << (x[1]/10) << ",1.2,1.6,1\n" ;
      parasfile << "car_minDist," << (x[2]/100) << ",0.3,0.5,1.5\n" ;
      parasfile << "car_sensitivityFactor," << (x[3]/100) << ",0," << (x[3]/100) << "," << (x[3]/100) << '\n';

      parasfile << "truck_maxAccel," << (x[4]/100) << ",0.5,0.6,1.8\n" ;
      parasfile << "truck_reactionTime," << (x[5]/10) << ",1.3,1.7,1\n" ;
      parasfile << "truck_minDist," << (x[6]/100) << ",0.5,1,2.5\n" ;
      parasfile << "truck_sensitivityFactor," << (x[7]/100) << ",0," << (x[7]/100) << "," << (x[7]/100) << '\n';
